<template>
  ...
  <nav aria-label="breadcrumb" style="--bs-breadcrumb-divider: '>';">
    <ol class="breadcrumb">
      <li class="breadcrumb-item active" aria-current="page">SSR 卡片整理</li>
    </ol>
  </nav>
  </div>

  <div class="row g-1 mb-2 row-cols-xl-3 row-cols-lg-2 row-cols-1">
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">所有 SSR 卡片</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <SSRModal :cards="modalCards" :japanese="japanese"/>
        </div>
      </div>
    </div>
  </div>
...
import SSRModal from './SSRModal.vue'
...
export default {
        ...
        return {
            ...
            filters: {
                'cardType': {
                    ...
                    'label': '卡片類型',
                    'enabled': true,
                    'options': [{'val': 0, 'text': '常駐'}, {'val': 1, 'text': '期間限定'},  {'val': 2, 'text': 'FES限定'}],
                    'selected': [],
                },
            },
            sorts: {
    ...
    methods: {
        updatePage: function() {
            this.$api.getIdolSSRCards().then((res) => {

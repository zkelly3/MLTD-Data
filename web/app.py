...
def get_idol_info(id):
    ...
    sql_cards = "SELECT id, jp_name, as_name, rare, jp_time, as_time FROM `Card` WHERE (IID = %s) ORDER BY jp_time, as_time, card_id"
    connection = connect()
    with connection.cursor() as cursor:
...
def get_card_info_local(id, local):
    sql_e_col = ""
    sql_e_pst = ""
    sql_e_ann = ""
    sql_e_oth = ""
    ...
    sql_gasha = """SELECT Gasha.id AS id, Gasha.{name} AS name, Gasha.{start} AS start, Gasha.{over} AS over, Gasha.type AS gasha_type
    FROM `GashaToCard` INNER JOIN `Gasha` ON GashaToCard.GID = Gasha.id
    WHERE (GashaToCard.CID = %s AND Gasha.{start} IS NOT NULL) ORDER BY Gasha.{start}"""
    sql_not_up = "SELECT id, {name} AS name, {start} AS start, {over} AS over, type AS gasha_type FROM `Gasha` WHERE ({start} = %s AND NOT type = 5)"
    sql_ls = "SELECT id, {name} AS name, {description} AS description, ssr, sr, r FROM `LeaderSkill` WHERE (id = %s)"
    ...
    with connection.cursor() as cursor:
        ...
        if not card:
            ...
            if aquire_id == 0:
                ...
                cursor.execute(n_sql_gasha, (id))
                gashas = cursor.fetchall()
                if not gashas:
                    cursor.execute(n_sql_not_up, (card['time']))
                    gashas = cursor.fetchall()
                for gasha in gashas:
                    gasha['start'] = gasha['start'].replace(tzinfo=timezone(timedelta(hours=local['ver_time']))).timestamp()
                    gasha['over'] = gasha['over'].replace(tzinfo=timezone(timedelta(hours=local['ver_time']))).timestamp()
